...
const filePath = path + `/Daily Logs/${formattedDate}.md`;
...
if (FileManager.local().fileExists(filePath)) {
  ...
  let processedTodos = uncompletedTodos.map((todo) =>
    removeTags(removeLinks(todo))
  );
  if (param != "default") {
...
if (param == "default") {
  ...
  addButton.url =
    "obsidian://advanced-uri?vault=Notes&commandid=quickadd%253Achoice%253A6bbad5c2-5846-4cb9-ad3a-3151327ea8c8";
}
